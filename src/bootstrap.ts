 ...
 */
export function guardBraceTemplates(text: string): string {
  ...
  console.warn(
    '[dsh-superpowers] escaped ' + (openCount + closeCount) + ' brace template occurrence(s) ' +
 ...
 */
export function buildBootstrap(skillsDir: string): string | undefined {
...
const bootstrapCache = new Map<string, string | undefined>();

/** `buildBootstrap` with a module-level per-directory cache. */
export function getBootstrap(skillsDir: string): string | undefined {
